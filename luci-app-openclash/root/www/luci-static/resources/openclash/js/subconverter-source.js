(function(window, document) {
	...
	var defaults = {
		...
		rawHint: 'Edit the original address directly. Unsupported parameters are preserved.',
	...
	function splitSources(value) {
		...
		var result = [];
		var separator = /\|(?=(?:<?(?:tag|provider|interval|proxy_direct):|[a-z][a-z0-9+.-]*:\/\/))/ig;
		lines.forEach(function(line) {
			line.split(separator).forEach(function(item) {
				item = item.trim();
	...
	function maybeDecodePrefixedSource(value) {
		if (!/^(?:%3c)?(?:tag|provider|interval|proxy_direct)(?:%3a|:)/i.test(value)) return value;
		try {
	...
	function parseSource(value) {
		...
		var remainder = maybeDecodePrefixedSource(original).trim();
		var item = {
			url: '',
			tag: '',
			provider: '',
	...
	function plainSources(items) {
		return items.map(function(item) { return item.url.trim(); }).filter(Boolean).join('\n');
	}
	...
	function init(options) {
		...
		var addButton = createElement('button', 'btn cbi-button cbi-button-positive', labels.add);
		addButton.type = 'button';
		var batchButton = createElement('button', 'btn cbi-button', labels.batch);
		batchButton.type = 'button';
		var rawButton = createElement('button', 'btn cbi-button', labels.raw);
		...
		var cancelButton = createElement('button', 'btn cbi-button', labels.cancel);
		...
		function render() {
			...
			items.forEach(function(item, index) {
				...
				var cardHeading = createElement('div', 'subconverter-source-item-heading');
				cardHeading.appendChild(createElement('strong', '', labels.source + ' ' + (index + 1)));
				if (items.length > 1) {
					...
					cardHeading.appendChild(removeButton);
				}
				...
				var urlField = makeField(labels.url, item.url, 'text', function(value) {
					item.url = value;
					markDirty(item);
					updateRemoteFields();
				});
				urlField.field.classList.add('is-wide');
		...
		function showRaw(messageText, fallback) {
			active = false;
			rawMode = true;
			rawMessage = messageText || labels.rawHint;
